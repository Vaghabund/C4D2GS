...
2. Select (or make active) the object you want to capture.
3. Run this script from the Cinema 4D Script Manager
   (Script ▸ Script Manager ▸ Execute).

The script will:
- Build a sphere of cameras around the active object.
- Set up an animated render camera that visits every viewpoint.
- Configure the Render Settings to output one PNG per viewpoint.
- Optionally export a camera_poses.json file (nerfstudio format).
- Optionally export synthetic COLMAP data files
...
│   ├── gs_0001.png
│   ...
├── camera_poses.json          (if EXPORT_CAMERA_POSES_JSON = True)
├── cameras.txt                (if EXPORT_COLMAP_DATA = True)
├── images.txt                 (if EXPORT_COLMAP_DATA = True)
└── points3D.txt               (if EXPORT_COLMAP_DATA = True)
"""
...
RESOLUTION_Y = 1080
FPS = 30
ENABLE_STRAIGHT_ALPHA = True    # If True, render with straight (non-premultiplied) alpha.
...
def _render_output_pattern():
    """Render path stem — C4D appends frame numbers to yield gs_0000.png etc."""
...
def export_colmap_data(world_points, target_pos, doc, target_obj,
    ...
    """Write cameras.txt / images.txt / points3D.txt to OUTPUT_PATH.

    Returns a result dict, or raises ValueError on failure.
    """
    if not world_points:
        return None
