...
def run_pipeline(doc, settings, target_obj, create_output_dirs=True):
    ...
    try:
        ...
        if settings.create_anim_cam:
            ...
            render_cam.SetRelPos(world_pts[0] - target_pos)
...
def generate_space_clusters(doc, settings, anchors):
    """
    Generate camera clusters for Space mode based on anchor points.

    :param doc: The Cinema 4D document.
    :param settings: Plugin settings.
    :param anchors: List of anchor points (world positions).
...
def generate_anchors(doc, settings):
    """
    Generate anchor points based on the selected placement mode.

    :param doc: The Cinema 4D document.
    :param settings: Plugin settings.
    :return: List of anchor points (world positions).
    """
    anchors = []

    if settings.anchor_mode == 0:  # Auto Placement
    ...
    elif settings.anchor_mode == 1:  # Manual Placement
...
def generate_spherical_cameras(doc, settings, anchors):
    """
    Generate spherical cameras at each anchor point for equirectangular rendering.

    :param doc: The Cinema 4D document.
    :param settings: Plugin settings.
    :param anchors: List of anchor points (world positions).
    :return: List of spherical cameras.
    """
    spherical_cameras = []
    ...
    for anchor in anchors:
        ...
        cam[c4d.CAMERA_PROJECTION] = c4d.PcameraSpherical  # Set projection to spherical
